...
function getConfigUrl() {
  ...
  switch (lang) {
    ...
    default:
      return INDONESIAN_CONFIG_URL;
  }
}

function i18n(text) {
  var lang = getUrlParameterByName('lang') || LANG_INDONESIAN;
...
function loadDictionary() {
  var request = new XMLHttpRequest();
  // Ensure absolute path for dictionary
  var dictUrl = LANGUAGE_CONFIG.DICTIONARY_URL;
  if (!dictUrl.startsWith('/')) {
    dictUrl = '/dict/' + dictUrl.replace('dict/', '');
  }
  request.open("GET", dictUrl, true);
  request.onreadystatechange = function()
  {
    if (request.readyState === 4) {
...
function showLetterInput(elem) {
  ...
  input_container.innerHTML = i18n('Huruf mana yang ingin Anda letakkan di sini?') + "<br><div class='input_letter'>" + PLAYER_1_LETTERS.join("</div><div class='input_letter'>") + "</div>";
...
function updatePlayButton() {
  ...
  if (points) {
    document.getElementById("move").innerHTML = i18n("Mainkan") + " (" + i18n("untuk") + " " + points + " " + i18n("poin") + ")";
    document.getElementById("move").disabled = false;
  } else {
    document.getElementById("move").innerHTML = i18n("Mainkan");
...
function shuffleLetters() {
  ...
  for (var i = PLAYER_1_LETTERS.length - 1; i > 0; i--) {
    var j = Math.floor(Math.random() * (i + 1));
    var temp = PLAYER_1_LETTERS[i];
    PLAYER_1_LETTERS[i] = PLAYER_1_LETTERS[j];
    PLAYER_1_LETTERS[j] = temp;
  }
  printPlayersLetters();
}
...
function printBoard() {
  for (var i=0; i<15; i++) {
    for (var j=0; j<15; j++) {
      ...
      var index = i * 15 + j;
      var letter = BOARD_LETTERS[index];
      field.innerHTML = letter;

      // Hide star on start square (center) if there's a letter
      if (index === 112 && field.classList.contains('start')) { // Center square (row 8, col 8)
        if (letter !== '') {
          field.classList.add('has-letter');
        } else {
          field.classList.remove('has-letter');
        }
      }
      ...
      if (TO_BE_PLAYED_BOARD_LETTER_INDEXES.indexOf(index) !== -1) {
      ...
      if (LETTERS_PLAYED_BY_KI_INDEXES.indexOf(index) !== -1) {
...
function onFinishMoveClick() {
  ...
  printPlayersLetters();
  printBoard(); // This will hide the star if a letter is on center square
  startKiMove();
}
...
function onSelectSwapTilesClicked() {
  ...
  button.innerHTML = i18n('Pilih huruf yang ingin ditukar, lalu klik di sini');
...
function onPerformSwapTiles() {
  ...
  var button = document.getElementById('pass');
  button.innerHTML = i18n('Tukar Huruf (Lewati)');
...
function endGame() {
  ...
  document.getElementById("input_container").innerHTML='Permainan selesai <a href="./" style="color:#ff9900">Mulai permainan baru</a>';
  document.getElementById("input_container").style.display= "block";

  document.getElementById("move").disabled = true;
  document.getElementById('pass').disabled = true;

  var winText = i18n('Anda menang.');
  var looseText = i18n('Anda kalah.');
  var resultText = PLAYER_1_POINTS > PLAYER_2_POINTS ? winText : looseText;

  alert(
    i18n('Permainan selesai.') + '\n' +
    resultText + '\n' +
    i18n("ANDA") + ": " + PLAYER_1_POINTS + ' ' + i18n("poin") + '\n' +
    i18n("AI") + ": " + PLAYER_2_POINTS + ' ' + i18n("poin")
  );
}
